...
if (process.env.SERVERS) {
  ...
  localhost = true;
  console.log('using localhost servers');
}
...
const NKEY_USER = process.env.NKEY_USER || 'UBKFQKEGMJSWLBDXGWPCINWMGZA4GICY6B56QYZU2EQ6APC65CWY2GTG';
const NKEY_SEED_VALUE = process.env.NKEY_SEED || 'SUADIW6UXY36QPGEVTHTRKE7ANELXEEH3GLPLK6SZW72O6VBYZKEJCSNWA';

const NKEY_SEED = new TextEncoder().encode(NKEY_SEED_VALUE);

// for localhost, these are the admin credentials, use these to create the stream
const NATS_ADMIN_USER = 'admin';
const NATS_ADMIN_PASSWORD = 'password';
...
const createStream = async () => {
  try {
    console.log('Establishing connection with admin credentials for stream creation...');
    const adminNC = await natsConnect({
      servers: servers,
      reconnectTimeWait: 10 * 1000, // 10s
      user: NATS_ADMIN_USER,
      pass: NATS_ADMIN_PASSWORD
    });

    const adminJs = adminNC.jetstream();
    const adminJsm = await adminJs.jetstreamManager();

    console.log(`Checking if stream "${STREAM_NAME}" exists...`);
    const streams = await adminJsm.streams.list().next();
    const streamExists = streams.some((stream) => stream.config.name === STREAM_NAME);

    if (!streamExists) {
      console.log(`Stream "${STREAM_NAME}" does not exist. Creating it...`);
      await adminJsm.streams.add({
        name: STREAM_NAME,
    ...
    // Close the admin connection after stream creation
    await adminNC.close();
  } catch (e) {
...
const main = async () => {
  if (localhost) {
    // running locally, let's set up a stream...
    // there won't be any messages in the stream, but we can create it to test the connection.
    await createStream();
  }
  await init();
  await pull();
